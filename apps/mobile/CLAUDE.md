...
- `aiScore` 없음
- `src/api/feed.ts` DEV mock 이미 제거됨 — 실제 백엔드 연동 상태
...
### Session Bundle — 번들 생성 + 템플릿 플로우 연결
여러 세션(plannedSessionCount > 1)을 완료했을 때 하나의 번들로 묶어 템플릿을 만드는 전체 흐름 구현.

**신규 파일**
- `src/api/session-bundle.ts` — `createSessionBundle`, `getSessionBundle` API 클라이언트

**수정 파일**
- `src/features/session/store.ts` — `completedSessionIds: string[]` 상태 + `addCompletedSessionId` 액션 추가
- `src/features/session/screens/SessionActiveScreen.tsx` — 세션 완료 시 `addCompletedSessionId(sessionId)` 호출
- `src/features/session/mutations.ts` — `useCreateSessionBundleMutation` 추가
- `src/features/session/queries.ts` — `useSessionBundleQuery(bundleId)` 추가
- `src/features/session/hooks.ts` — 위 두 항목 re-export
- `src/features/session/screens/SessionCompleteScreen.tsx`:
  - 마지막 세션 완료 시 (plannedSessionCount > 1 && !hasMoreSessions) 번들 섹션 표시
  - 번들 제목 입력 (기본값: 세션 topic)
  - `Bundle & Archive` → 번들 생성 → `sharePreviewModal({ bundleId })` 이동
  - 단일 세션은 기존 `Save & Archive` 흐름 유지

**SharePreview 번들 모드 지원**
- `src/constants/routes.ts` — `sharePreviewModal` 시그니처를 `{ sessionId?, bundleId?, quizScore? }` object로 변경, 기존 5개 call site 모두 업데이트
- `src/features/share/types.ts` — `ShareModel`에 `sourceType`, `bundleId` 필드 추가
- `src/features/share/helpers.ts` — `buildBundleShareModel` 추가 (번들 → NikeTemplate 모델 변환)
- `src/features/share/screens/SharePreviewScreen.tsx`:
  - `bundleId` 파라미터 감지 → `isBundleMode` 분기
  - bundle mode: `useSessionBundleQuery` + `buildBundleShareModel`
  - `handleArchive` — bundle mode는 `sourceType: 'sessionBundle'`, `bundleId` 전달

**번들 플로우 전체 경로**
```
세션 1 → 세션 2 → ... → 마지막 세션 완료
  ↓ SessionCompleteScreen (번들 제목 입력)
  ↓ POST /v1/session-bundles
  ↓ SharePreviewScreen (bundleId, 템플릿 렌더링)
  ↓ Archive 버튼 → highlight 저장 (sourceType: sessionBundle)
  ↓ ArchiveHighlightDetail
```

---

### Share — Supabase Storage 실제 연결 완료
- 백엔드 presign이 실제 Supabase Storage signed URL 반환
- `uploadImageToStorage` PUT → Supabase Storage `content` 버킷에 실제 저장
- `renderedImageUrl` → `https://wmivfpqzvuxabgekbmht.supabase.co/storage/v1/object/public/content/...` 정상 작동
- 프론트 코드 변경 없음 — `src/api/archive.ts`의 presign + upload + cache 플로우 그대로 사용
